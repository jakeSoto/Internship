...
# ((mCherry / MEASURED_value) - min) / max
def newNormalized(dataSet, measuredVals) -> ([], []):
    newNorm = np.array(dataSet)
    finalNorm = np.array(dataSet)

    for i, value in enumerate(dataSet):
        newNorm[i] = value / measuredVals[i]
        finalNorm[i] = (newNorm[i] - np.min(newNorm[i])) / np.max(newNorm[i])

    return newNorm, finalNorm
